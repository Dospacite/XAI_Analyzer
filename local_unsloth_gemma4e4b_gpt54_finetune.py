#!/usr/bin/env python3
# %% [markdown]
# # Local Unsloth Gemma 4 E4B Fine-Tuning on `gpt54_out_predictions.jsonl`
#
# Notebook-style local pipeline with `# %%` cell markers.
#
# What this does:
# - Loads `gpt54_out_predictions.jsonl`
# - Uses `request.website_observation` as the model input
# - Uses `output_json` as the supervised fine-tuning target
# - Fine-tunes `unsloth/gemma-4-E4B-it-unsloth-bnb-4bit` with LoRA / QLoRA
# - Benchmarks the raw model and the fine-tuned adapter
#
# Notes:
# - This is local, not Colab-oriented.
# - You must have accepted Gemma's Hugging Face license and be authenticated if required.
# - The dataset contains some `label != output_json.verdict` rows, so this script reports metrics
#   against both the original label and the teacher target verdict.
...
if INSTALL_DEPS:
    # This pipeline does not log to wandb. Removing a broken wandb install
    # avoids TRL import failures caused by incompatible preinstalled packages.
...
DATA_JSONL = PROJECT_DIR / "gpt54_out_predictions.jsonl"
RUN_DIR = PROJECT_DIR / "runs" / "gemma4e4b_gpt54_unsloth"
...
TEST_SIZE = 0.10
VAL_SIZE = 0.05
STRATIFY_BY = "label"  # "label" or "target_verdict"
BALANCE_CLASSES = False

ALIGN_TARGET_VERDICT_TO_LABEL = False
USE_ONLY_CORRECT_PREDICTIONS = True
...
DO_RAW_BENCHMARK = True
MAX_EVAL_EXAMPLES_RAW = 50
MAX_EVAL_EXAMPLES_TUNED = 300
GEN_MAX_NEW_TOKENS = 256
EVAL_BATCH_SIZE = 4
PROMPT_TOKEN_BUDGET = 1536

EXPORT_MERGED_16BIT = False
EXPORT_GGUF_BF16 = False
...
def parse_prediction_output(text: str) -> tuple[dict[str, Any] | None, str]:
    strict = normalize_prediction_payload(extract_first_json_object(text))
    if strict is not None:
        return strict, "strict_json"
    repaired = repair_prediction_payload(text)
...
if BALANCE_CLASSES:
    ...
    min_count = min(len(items) for key, items in grouped.items() if key in {"phishing", "benign"})
    balanced_records = []
    for key, items in grouped.items():
        if key in {"phishing", "benign"}:
            random.shuffle(items)
            balanced_records.extend(items[:min_count])
    random.shuffle(balanced_records)
else:
    balanced_records = records[:]

train_records, test_records = stratified_split(balanced_records, TEST_SIZE, SEED, split_key)
...
# %%
# Load Gemma 4 E4B with Unsloth.
if not torch.cuda.is_available():
    raise RuntimeError("CUDA is required for this Unsloth Gemma 4 fine-tuning pipeline.")
...
for model_name in TRAINABLE_MODEL_CANDIDATES:
    ...
    raise RuntimeError("Could not load any trainable Gemma 4 E4B candidate.") from last_load_error
...
print("Loaded:", TRAINABLE_MODEL_NAME)
print("Tokenizer padding_side:", getattr(text_tokenizer, "padding_side", None))
...
def benchmark(records_to_eval: list[dict[str, Any]], name: str, limit: int) -> dict[str, Any]:
    ...
    print("\nReport vs label:\n", metrics["classification_report_vs_label"])
    print("\nReport vs target:\n", metrics["classification_report_vs_target"])
    return metrics

# %%
# Benchmark raw model before fine-tuning.
...
) if RESPONSE_TRAINING_ONLY else trainer

(RUN_DIR / "training_config.json").write_text(
    json.dumps(
        {
            "model_candidates": TRAINABLE_MODEL_CANDIDATES,
...
# %%
# Optional merged model / GGUF export.
...
# %%
# Quick manual inference on one held-out example.
